    ...
    switch (assessment.stage) {
      ...
      case 1:
        if(numCompJudge(comparisons, assessor)>=maxComp+_.get(assessment, "comparisonsNum.stage[1]", 17))
          return { messages: ["You can't make any more comparisons at the moment."] };
        if(_.find(representations, function(representation){
            return (!_.get(representation, "ability.value", null)); //CHANGE TO BETTER TEST!!!
          }) ){
        ...
        break;
      default:
        throw new Error("assessment stage has no value");
        break;
    }
...
function selectBStage2(representations, comparisons, representationA) {
  ...
  return selectB(subsetNComp.subsetNcomp(subset), cuttingPoint.ability.value);
}
