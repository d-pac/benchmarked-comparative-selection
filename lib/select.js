...
function prepareRepresentations( representations ){
  return _.map( representations, function( representation ){
    if( !representation.compared || !_.isArray( representation.compared ) ){
      //we're gonna play safe ball here, to avoid problems with `representation.compared` being `undefined` instead of `[]`
...
function select( representations,
  ...
  representations = prepareRepresentations( representations );
