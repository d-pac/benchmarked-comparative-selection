...
function select( representations,
  ...
  var benchmarks = _.filter(representations, function(representation){
    return representation.rankType === "benchmark"
  });
  ...
  _.each(benchmarks, function(benchmark){
    counted[benchmark._id]=0;
      _.each(representations, function(representation){
        if(representation.rankType === "ranked"){
          if(representation.closeTo === benchmark._id){
            ++counted[benchmark._id];
          }
        }
      })
  });

  if(_.find(counted, function(counter){
      return counter<1})<1 ){
  ...
  if(_.find(representations, function(representation){  //COULD WE NOT DO THIS IN SWICH?
    ...
    switch (assessment.stage) {
      case 1:
        ...
        break;
      case 2:
        //representationA should have an ability.value field
