...
module.exports = function selectReprA(representation, comparisons, assessorId) {
//lijst maken van repr die toRank zijn
  var reprToRank = [];
  _.each(representation, function (repobjects) {
    if (repobjects.rankType === "to rank") {
      reprToRank.push(repobjects._id);
    }
  });
...
// REMARK: this should only be calculated for the comparisons done by this judge OK!


  _.each(comparisons, function (compobjects) {
    //hier user01 nog vervangen door judge die paar wenst wanneer alles samenzet
  ...
  //var commoncount = _.countBy([reps], 'rep02');
  commoncount = {};
...
// REMARK: this should only be calculated for the "toRank" representations
  ...
  var ReprToRankSorted = _.sortBy( _.filter( representation, { rankType : "to rank"} ), "compared.length" );

  console.log("ReprToRankSorted", ReprToRankSorted);

//create variable represA
// issues: 1 sorteert niet juist, own het stuk na de +

  //var sortedReprA = _.sortBy( TalliedReprA.map, "weight" );

  /*var commoncountId = [];

  _.each(representation, function (repid) {
    if(commoncount.indexOf(repid._id)>-1){
      commoncountId.push(repid._id);
    }
  });
*/
