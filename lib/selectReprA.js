...
//lijst maken van repr die toRank zijn
  ...
  if(!_.isArray(representation) || representation.length<3) {
         throw new Error("representation should be an array with a minimum length of 1")
  }
  ...
  if(!_.isArray(reprToRank) || reprToRank.length<2) {
    throw new Error("the to rank representation should be an array with a minimum length of 1")
  }
  console.log("reprToRank", reprToRank);
>>>>>>> origin/master

  //lijst maken van repr die toRank zijn
  var reprToRank = _.pluck(_.filter(representation,{ rankType: "to rank"}), "_id");
  console.log("reprToRank", reprToRank);
